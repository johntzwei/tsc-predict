...
EXP02_RESULTS_DIR = Path(__file__).parent.parent / "02_winogrande_predict" / "results"
FIGURES_DIR = Path(__file__).parent / "figures"
FIGURES_DIR.mkdir(exist_ok=True)


def main():
    # Discover all probe prediction files (exp 03 + exp 02)
    pred_files_03 = sorted(RESULTS_DIR.glob("test_predictions_*.npz"))
    pred_files_02 = sorted(EXP02_RESULTS_DIR.glob("test_predictions_*.npz"))
    pred_files = pred_files_02 + pred_files_03
    probe_names = [
        f"frozen_{f.stem.removeprefix('test_predictions_')}" for f in pred_files_02
    ] + [f.stem.removeprefix("test_predictions_") for f in pred_files_03]
